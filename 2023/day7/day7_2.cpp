...
int main() {
  ...
  for (int j = 0; j < game.size(); j++) {
    ...
    int Jcounter = 0;
    bool added = false;
    // Check if there is a J in the hand.
    for (auto it : count[j]) {
      if (it.first == 'J') {
        Jcounter = it.second;
      }
    ...
    for (auto it : count[j]) {
      ...
      // Add the counter number if there is a pair or more.
      if (it.second > 1 && !added) {
        it.second += Jcounter;
        added = true;
      }
    ...
    // If there are no pairs add points depening on how many Js on hand.
    if (game[j].points <= 1) {
      switch (Jcounter) {
      case 1:
        game[j].points += 2;
